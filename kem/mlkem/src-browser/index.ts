  ...
  if (!mlkemModuleInstance) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (typeof (window as any).Module === 'function') {
        ...
        const moduleArg = {
          locateFile: (path: string) => {
            if (path.endsWith('.wasm')) {
              return '../../dist-browser/kem/mlkem/mlkem_wrapper.wasm';
            }
            return path;
          }
        };
        ...
        script.src = '../../dist-browser/kem/mlkem/mlkem_wrapper.js';
        script.onload = async () => {
          try {
            const moduleArg = {
              locateFile: (path: string) => {
        ...
        script.onerror = () => reject(new Error('Failed to load ML-KEM wrapper script'));
...
async function createWrappers(): Promise<void> {
  ...
  // Create wrappers for all ML-KEM variants
  const variants = [512, 768, 1024]; // ML-KEM variants
  for (const variant of variants) {
    try {
      mlkemWrappers[`mlkem_${variant}`] = await createMlkemWrapper(variant);
    } catch (error) {
      console.warn(`Failed to create wrapper for ML-KEM ${variant}:`, error);
    }
  }
}
